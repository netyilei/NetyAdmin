...
Email 配置（`email_config`）**不走缓存，每次发送直接从数据库读取**：

```
emailDriver.resolveConfig(ctx)
  → dbConfigProvider.GetByGroup(ctx, "email_config")
    → systemRepo.ConfigRepository.GetByGroup(ctx, "email_config")
      → GORM: WHERE group_name = 'email_config'
```

这是**有意的设计选择**，原因如下：
- 邮件配置在后台管理页面中随时可能修改，需要实时生效
- 消息发送是低频操作，不存在性能瓶颈
- 如果走缓存，则需要显式失效逻辑，容易出现配置更新但缓存没失效的问题
- 数据库是唯一真相源，多机部署时天然一致
