...
interface Emits {
  (e: 'submitted', menuData?: SystemManage.Menu): void;
}
...
const localIconOptions = localIcons.map(item => ({
  value: item,
  label: () =>
    h('div', { class: 'flex-y-center gap-16px' }, [
      h(SvgIcon, { localIcon: item, class: 'text-icon' }),
      h('span', item)
    ])
}));
...
function handleUpdateI18nKeyByRouteName() {
  if (model.value.routeName) {
    model.value.i18nKey = `route.${model.value.routeName}` as App.I18n.FlexibleI18nKey;
  } else {
...
function handleCreateButton() {
  const buttonItem: SystemManage.MenuButton = {
    code: '',
    desc: ''
  };
...
async function handleSubmit() {
  ...
  await useOperation(props.operateType, loading, {
    edit: () => fetchUpdateMenu(params as SystemManage.Menu),
    add: () => fetchAddMenu(params as SystemManage.Menu),
    onSuccess: () => {
      closeDrawer();
      emit('submitted', params as unknown as SystemManage.Menu);
    }
  });
}
      ...
      <NForm ref="formRef" :model="model" :rules="rules" label-placement="left" :label-width="100">
        <NGrid responsive="screen" item-responsive>
          <NFormItemGi span="24 m:12" :label="$t('page.manage.menu.menuType')" path="type">
            <AppDictRadioGroup v-model:value="model.type" dict-code="menu_type" :disabled="disabledMenuType" />
          </NFormItemGi>
          <NFormItemGi span="24 m:12" :label="$t('page.manage.menu.menuName')" path="name">
