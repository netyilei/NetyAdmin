...
async function handleSubmit() {
  ...
  if (props.operateType === 'add') {
    const { error } = await addTemplate(model);
    if (!error) {
    ...
    const { error } = await updateTemplate(model);
    if (!error) {
    ...
    <NForm ref="formRef" :model="model" :rules="rules" label-placement="left" :label-width="120">
      <NGrid :cols="24" :x-gap="18">
        ...
        <NFormItemGi :span="24" :label="$t('page.messageHub.template.providerTplId')" path="providerTplId">
          <NInput v-model:value="model.providerTplId" :placeholder="$t('page.messageHub.template.form.providerTplIdPlaceholder')" />
        </NFormItemGi>
      </NGrid>
    </NForm>
    <template #footer>
