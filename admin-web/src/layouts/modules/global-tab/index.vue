...
import { PageTab } from '@na/materials';
...
function handleTabMouseMiddleClick(e: MouseEvent, tab: App.Global.Tab) {
  if (e.button === 1 && !tabStore.isTabRetain(tab.id)) {
    e.preventDefault();
    tabStore.removeTab(tab.id);
  }
}
      ...
      <BetterScroll ref="bsScroll" :options="{ scrollX: true, scrollY: false, click: !isPCFlag }" @click="removeFocus">
        ...
        >
          <PageTab
            ...
            @mouseup="handleTabMouseMiddleClick($event, tab)"
